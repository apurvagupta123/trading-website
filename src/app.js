...
const FINNHUB_API_KEY = 'd81lae1r01qrojfc2i2gd81lae1r01qrojfc2i30';
...
// Mock Financial Data (Fallback only)
const mockCompanies = {
  'AAPL': {
    name: 'Apple Inc.',
    symbol: 'AAPL',
  ...
  'GOOGL': {
    ...
    low52w: 102.21,
    volume: 23145000,
    description: 'Alphabet Inc. is a technology conglomerate that operates Google, one of the world\'s largest search engines.',
    overview: {
      founded: 1998,
  ...
  'MSFT': {
    ...
    low52w: 310.21,
    volume: 18956000,
    description: 'Microsoft Corporation develops, manufactures, licenses, and supports a wide range of products and services.',
    overview: {
      founded: 1975,
...
async function searchCompany(query) {
  ...
  try {
    ...
    const data = await fetchCompanyDataFromAPI(symbol);
    
    if (data) {
      console.log(`Successfully fetched real data for ${symbol}`, data);
      loadCompanyData(data);
      searchInput.disabled = false;
      searchInput.placeholder = 'Search company (e.g., AAPL, GOOGL, MSFT)';
      return;
    }
  } catch (error) {
  ...
  if (mockCompanies[symbol]) {
    ...
    const company = mockCompanies[symbol];
    company.isMockData = true;
    loadCompanyData(company);
  } else {
    alert(`Company ${symbol} not found. Try: AAPL, GOOGL, MSFT, TSLA, RELIANCE, INFY, BMW, ASML, etc.`);
  }
...
async function fetchCompanyDataFromAPI(symbol) {
  try {
    ...
    // Check cache first
    if (apiCache[symbol] && (Date.now() - apiCache[symbol].timestamp) < CACHE_DURATION) {
    ...
    if (!profileData || profileData.error) {
      console.error('Profile fetch failed or error returned');
      return null;
    }
    ...
    if (!quoteData || quoteData.error) {
      console.error('Quote fetch failed or error returned');
      return null;
    }
    ...
    console.log(`Financials response:`, financialsData);
    ...
    const company = {
      ...
      high52w: profileData.valuation?.week52High || quoteData.h || 0,
      low52w: profileData.valuation?.week52Low || quoteData.l || 0,
      volume: quoteData.v || 0,
      avgVolume: quoteData.vo || 0,
      description: profileData.description || `${symbol} is a public company trading on ${profileData.exchange}`,
      
      overview: {
        founded: profileData.ipo ? new Date(profileData.ipo).getFullYear() : 'N/A',
        headquarters: `${profileData.city}, ${profileData.country}` || 'N/A',
        ceo: profileData.CEO || 'N/A',
        employees: 'Real-time data',
      ...
      valuation: {
        ...
        roe: profileData.valuation?.roe ? (profileData.valuation.roe * 100).toFixed(2) : 'N/A',
        roa: profileData.valuation?.roa ? (profileData.valuation.roa * 100).toFixed(2) : 'N/A',
        debtToEquity: profileData.valuation?.debtToEquityRatio || 'N/A'
      },

      technicals: {
        ma50: (quoteData.c + quoteData.o) / 2 || 0,
        ma200: (quoteData.c + quoteData.l + quoteData.h) / 3 || 0,
    ...
    console.log(`Successfully processed ${symbol}:`, company);
    return company;

  } catch (error) {
    console.error('Exception in fetchCompanyDataFromAPI:', error);
    return null;
  }
}
...
function parseFinancials(data) {
  ...
  const financials = data.data.slice(0, 3).map(item => ({
    ...
    totalLiabilities: formatCurrency(item.totalLiabilities),
    shareholders: formatCurrency(item.shareholders),
    operatingCF: formatCurrency(item.operatingCashFlow),
    freeCashFlow: formatCurrency(item.freeCashFlow)
  }));

  return {
    incomeStatement: financials,
    balanceSheet: financials,
    cashFlow: financials
  };
}
...
function loadCompanyData(company) {
  ...
  if (header) {
    const dataSource = company.isRealData ? '🔴 LIVE' : '⚪ MOCK';
    header.innerHTML = `
      <div class="header-top">
        <div>
          <h1>${company.name}</h1>
          <p class="symbol-exchange">${company.symbol} • ${company.exchange} ${dataSource}</p>
        </div>
